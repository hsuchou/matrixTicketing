            ...
            threads[i] = new Thread(new Runnable() {
                public void run() {
                    ...
                    for (int k = 0; k < testnum; k++) {
                        ...
                        int cnt = 0;
                        if (isSequential) {
                            while (ThreadId.get() != barrier.value && exOthNotFin(threadnum, ThreadId.get())) {
                            }
                            SLOCK_TAKE();
                        }
                        ...
                        if (isSequential) {
                            ...
                            if (exOthNotFin(threadnum, ThreadId.get())) {
                                barrier.value = rand.nextInt(threadnum);
                                while (fin[barrier.value]) {
                                    barrier.value = rand.nextInt(threadnum);
                                }
                            }
                            SLOCK_GIVE();
